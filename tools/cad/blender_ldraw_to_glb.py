"""
Batch convert LDraw .dat files to GLB using Blender with ExportLDraw addon.
Run with: blender --background --python blender_ldraw_to_glb.py
...
Output: WSL path models/ldraw/glb/
"""
...
def process_ldraw(input_path, output_path):
    """Process single LDraw .dat file to GLB."""
    clear_scene()

    # Import using ExportLDraw addon
    # Operator: bpy.ops.ldraw_exporter.import_operator
    try:
...
def main():
    print("\n" + "=" * 60)
    print("Blender LDraw to GLB Batch Converter")
    print("=" * 60)
